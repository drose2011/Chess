...
/*
The Board class:
	- Stores the posisioning of the pieces in an array
	- checks for check and checkmate
	- recives the input from ChessFX of the clicked spots and decieds if it is allowed
...
public class Board {
	...
	// intitializes the starting board
	public Board(){
		String side="w";
		this.gameStatus=0;
	...
	public Board(Piece[] pieces, int turnnum){
		// I had a slight issue making a new board becasue instead of duplicating it 
	...
	public StackPane updatedBoard() {
		...
		//makes backgroud board with board colors to signify clicked piece and possible moves
		...
		// Overlays the chess pieces on top of board in an idetical button array, except with 
	...
	public void move(int index, Boolean checking){
		...
		if(isAllowed){
			if(this.pieces[this.clickedPieceIndex].toString().equals("king")&&!this.pieces[this.clickedPieceIndex].hasMoved()){
			...
			//promoting 
			if((index<8||index>55)&&this.pieces[this.clickedPieceIndex].toString().equals("pawn")){
	...
	public int checkCheck(int round,Boolean kingtest){
		for(int i=0;i<64;i++){
			if(this.pieces[i]!=null&&!(kingtest&&this.pieces[i].toString().equals("king"))){
		...
		for(int i=0;i<this.pieces.length;i++){
			if(this.pieces[i]!=null&&this.pieces[i].getSide()!=side){
				...
				for(int j=0;j<allow.length;j++){
					...
					//kinda recursivly calls checkCheck
